...
The optional tolerance parameter is the acceptable difference between
periapsis and apoapsis; once they are within this distance, the
...
#+begin_src ks
  set mpcirchere to { parameter toll is 100.
    mpadd({
      ...
      // if we are staging, wait for staging to complete.
      if ship:availablethrust<=0 return 1/10.

      lock fwd to // velocity direction for a CIRCULAR orbit here.
        vxcl(body:position,prograde:vector).

      lock vc to  // speed required for CIRCULAR orbit
        sqrt(body:mu/body:position:mag).

      lock dv to // desired change in velocity vector
        fwd:normalized*vc - velocity:orbit.

      lock maxa to // acceleration at max throttle
        ship:availablethrust/mass.

      lock cthr to // graceful throttle shutdown
        sqrt(dv:mag/maxa).

      lock eang to // cut throttle when pointed the wrong way.
